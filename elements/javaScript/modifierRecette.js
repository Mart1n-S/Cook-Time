...
var urlImage = document.getElementById("hiddenValue").value;
var photo = document.getElementById("hiddenValue2").value;
const input1 = document.getElementById("imageRecette2");
if(photo || input1){
...
var imageSize = document.getElementById("hiddenValue2").value;
if(urlImage !== null && urlImage !== '' ){
    ...
    inputHidden.name = 'previewImageModifier';

    const image = document.createElement("img");
    image.src = "./uploads/imagesRecettes/"+urlImage ;
    ...
    // Sinon on vérifie si un fichier à été séléctionné
...
var textareaCount = textareas.length;
    ...
    document.getElementById('textarea-container2').appendChild(div);
};
...
document.getElementsByTagName('form')[0].addEventListener('submit', function(event) {
    ...
    for (var i = 0; i < textareas.length - 1; i++) {
      if (textareas[i].value == '' && textareas[i+1].value != '') {
        ...
        document.getElementById('error-message2').textContent = 'Vous avez oublié une étape.';
...
function supprimerIngredient (elementId){
    ...
    for( let i = 0; i < containerGlobale.length; i ++){
        ...
        if(containerGlobale[i].children[0].firstChild.id.startsWith('modifierQuantite')){
            containerGlobale[i].children[0].firstChild.id = 'modifierQuantite' + (i + 4)
        }else{
        ...
        if(containerGlobale[i].children[1].firstChild.id.startsWith('modifierUniteMesure')){
            containerGlobale[i].children[1].firstChild.id = 'modifierUniteMesure' + (i + 4)
        }else{
        ...
        if(containerGlobale[i].children[2].firstChild.id.startsWith('modifierIngredient')){
            containerGlobale[i].children[2].firstChild.id = 'modifierIngredient' + (i + 4)
        }else{
...
function ajoutIngredient(){
        ...
        var selectsCount = selects.length;
            ...
            const select1 = document.getElementById('modifierUniteMesure1');
        ...
        // Ajoute les différents champs aux différentes div
            ...
            document.getElementById('inputContainerModifier').appendChild(div0);
...
const formulaire = document.querySelector('#formulaireModifierRecette');
...
$(document).ready(function() {
    $("#formulaireModifierRecette").submit(function() {
      // récupérer les valeurs saisies par l'utilisateur
      var titreRecette = $("#titreRecette2").val();
      var typeRecette = $("#typeRecette2").val();
      var photoRecette = $("#imageRecette2").val();

      // assigner les valeurs saisies par l'utilisateur aux champs de formulaire correspondants
      $("#titreRecette2").val(titreRecette);
      $("#typeRecette2").val(typeRecette);
      $("#imageRecette2").val(photoRecette);
    });
  });